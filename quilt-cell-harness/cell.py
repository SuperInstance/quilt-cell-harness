...
class Engine:
    ...
    substrates: dict = field(default_factory=lambda: {
        # substrate_name : callable(str) -> str
        "stub_llm": lambda prompt: f"[llm-stub:{prompt[:40]}...]",
        "echo":     lambda prompt: prompt,
        "reverse":  lambda prompt: prompt[::-1],
        "sha256":   lambda prompt: hashlib.sha256(prompt.encode()).hexdigest(),
    })
